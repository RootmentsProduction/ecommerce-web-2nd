...
let EmailService = EmailService_1 = class EmailService {
    ...
    async sendEmail(options) {
        ...
        if (this.sesClient) {
            try {
                ...
                this.logger.warn(`Failed to send email to ${to} via AWS SES (falling back to local file log): ${stack}`);
            }
        }
        if (!sentViaSes) {
            ...
            this.logger.log('==================================================');
            try {
                const artifactsDir = '/Users/abijithgkaimal/.gemini/antigravity-ide/brain/c83e1200-1990-4523-9d03-382b8d6125fb';
    ...
    async sendAdminOrderNotification(order, customer) {
        const adminEmail = this.configService.get('ADMIN_NOTIFICATION_EMAIL') || 'admin@jewelrybyzorucci.com';
    ...
    async sendLowStockAlert(sku, productName, variantName, currentStock) {
        const adminEmail = this.configService.get('ADMIN_NOTIFICATION_EMAIL') || 'admin@jewelrybyzorucci.com';
